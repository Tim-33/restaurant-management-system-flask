...
class RezervacijaService():
    ...
    def get_rezervacija_with_stol_data(self):
        try:
            sql_script = get_sql_script_from_file(RezervacijaSqlRoutesEnum.SELECT_WITH_STOL_DATA.value)
            self.cursor.execute(sql_script)
            data = self.cursor.fetchall()
            rezervacije = [
                {
    ...
    def get_active_rezervacija_with_stol_data(self):
        try:
            sql_script = get_sql_script_from_file(RezervacijaSqlRoutesEnum.SELECT_ACTIVE_WITH_STOL_DATA.value)
            self.cursor.execute(sql_script)
            data = self.cursor.fetchall()
            rezervacije = [
                {
